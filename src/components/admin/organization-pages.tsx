...
import { AddDepartmentDialog, AddEmployeeDialog } from "@/components/admin/org-forms";
...
export function EmployeesPage({ initialFilters = {} }: { initialFilters?: EmployeeFilters }) {
  const [query, setQuery] = useState(""); const [company, setCompany] = useState(initialFilters.company ?? ""); const [department, setDepartment] = useState(initialFilters.department ?? ""); const [unplaced, setUnplaced] = useState(initialFilters.unplaced ?? false); const [sort, setSort] = useState<keyof typeof COLUMN_LABELS>("name"); const [descending, setDescending] = useState(false); const [page, setPage] = useState(1); const [view, setView] = useState<"table" | "grid">("grid");
