...
export function AppearanceMenu() {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="governos-appearance-menu max-h-[calc(100vh-2rem)] max-w-2xl overflow-y-auto rounded-2xl p-0" aria-label="Appearance settings">
        <DialogHeader className="border-b border-border/70 px-6 py-5 text-left sm:px-7">
        ...
        <div className="space-y-5 px-6 py-6 sm:px-7 sm:py-7">
          <div>
            <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Mode</div>
            <div className="grid grid-cols-2 gap-2">
              {([{ label: "Dark", value: true, Icon: MoonIcon }, { label: "Light", value: false, Icon: SunIcon }] as const).map(({ label, value, Icon }) => (
                <button key={label} type="button" onClick={() => chooseTheme(value)} aria-pressed={isDark === value} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-left text-xs transition-colors ${isDark === value ? "border-primary bg-primary/10 text-primary" : "border-border/70 hover:bg-accent"}`}>
                  <Icon className="h-4 w-4" />{label}{isDark === value && <CheckIcon className="ml-auto h-3.5 w-3.5" />}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Wallpaper</div>
            <div className="grid max-h-64 grid-cols-3 gap-2 overflow-y-auto pr-1">
          {WALLPAPERS.map((item) => (
            <button key={item.id} type="button" onClick={() => chooseWallpaper(item)} className={`group relative overflow-hidden rounded-lg border text-left ${wallpaper === item.id ? "border-primary ring-1 ring-primary" : "border-border/70"}`}>
              <span className="relative block h-14 bg-cover bg-center" style={{ backgroundImage: item.value.startsWith("linear-gradient") ? wallpaperPreview(item.value) : undefined }}>
              ...
              <span className="flex items-center justify-between px-2 py-1.5 text-xs">{item.label}{wallpaper === item.id && <CheckIcon className="h-3.5 w-3.5 text-primary" />}</span>
            </button>
          ))}
          {wallpaper === "custom" && <div className="overflow-hidden rounded-lg border border-primary ring-1 ring-primary"><span className="block h-14 bg-cover bg-center" style={{ backgroundImage: `var(--governos-wallpaper)` }} /><span className="flex items-center justify-between px-2 py-1.5 text-xs">Custom <CheckIcon className="h-3.5 w-3.5 text-primary" /></span></div>}
            </div>
            <input ref={fileInput} type="file" accept="image/*" className="hidden" onChange={(event) => uploadWallpaper(event.target.files?.[0])} />
            <Button type="button" variant="outline" className="mt-3 h-9 w-full text-xs" onClick={() => fileInput.current?.click()}>Upload custom wallpaper</Button>
          </div>

          <div className="border-t border-border/70 pt-5">
            <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Colour</div>
            {(["Cool", "Green", "Warm", "Gradient", "Neutral"] as const).map((group) => (
              <div key={group} className="mb-2">
                <div className="py-1 text-[10px] text-muted-foreground">{group}</div>
            <div className="grid grid-cols-2 gap-1 sm:grid-cols-3">
            {BRANDS.filter((item) => item.group === group).map((brand) => (
              <button key={brand.id} type="button" onClick={() => chooseBrand(brand)} className="flex min-w-0 items-center gap-2 rounded-md px-2 py-2 text-left text-xs hover:bg-accent" style={{ fontFamily: `"${brand.font}", sans-serif` }}>
                <span className="h-5 w-5 shrink-0 rounded-md border border-border/70" style={{ background: gradientCss(brand) }} />
